@echo off
REM Script para iniciar o Portal Cativo no Windows
REM Requer privilégios de Administrador para porta 80 e DNS

echo ========================================
echo  Portal Cativo - Iniciando Servidor
echo ========================================
echo.

REM Verificar se está rodando como administrador
net session >nul 2>&1
if %errorLevel% neq 0 (
    echo [AVISO] Este script requer privilégios de Administrador
    echo [AVISO] Execute como Administrador para usar porta 80 e DNS
    echo.
    echo Pressione qualquer tecla para continuar mesmo assim...
    pause >nul
)

REM Carregar variáveis de ambiente do arquivo .env
if exist .env (
    echo [INFO] Carregando configurações do arquivo .env
    for /f "tokens=1,2 delims==" %%a in (.env) do (
        set "%%a=%%b"
    )
) else (
    echo [AVISO] Arquivo .env não encontrado
    echo [AVISO] Criando .env com valores padrão...
    echo LOCAL_IP=192.168.0.103 > .env
    echo PORT=80 >> .env
    echo ENABLE_DNS=true >> .env
    echo NODE_ENV=production >> .env
)

REM Configurar variáveis de ambiente padrão se não existirem
if not defined LOCAL_IP set LOCAL_IP=192.168.0.103
if not defined PORT set PORT=80
if not defined ENABLE_DNS set ENABLE_DNS=true
if not defined NODE_ENV set NODE_ENV=production

echo [INFO] Configuracoes:
echo   - IP Local: %LOCAL_IP%
echo   - Porta: %PORT%
echo   - DNS: %ENABLE_DNS%
echo   - Ambiente: %NODE_ENV%
echo.

REM Definir variáveis de ambiente para o Node.js
set LOCAL_IP=%LOCAL_IP%
set PORT=%PORT%
set ENABLE_DNS=%ENABLE_DNS%
set NODE_ENV=%NODE_ENV%

echo [INFO] Iniciando servidor...
echo.

REM Iniciar o servidor Node.js
node server/index.js

